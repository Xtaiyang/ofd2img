...
class OFDParser(object):
    ...
    def parser(self, ):
        ...
        if public_res_name:
            ...
            public_res_data = PublicResFileParser(public_xml_obj)()
            font_info = public_res_data.get("fonts", {})
            draw_param_info = public_res_data.get("draw_params", {})
        ...
        tpl_id_map = doc_root_info.get("tpl_id_map", {})
        ...
        if page_name:
            for index, _page in enumerate(page_name):
                ...
                try:
                    ...
                    traceback.print_exc()
                    page_size.append([])
                page_info = ContentFileParser(page_xml_obj, draw_params=draw_param_info)()
                
                # 处理页面引用的模版         
                page_tpls = page_info.get("tpls", [])
                if page_tpls:
                    for tpl_ref in page_tpls:
                        tpl_id = tpl_ref.get("TemplateID")
                        if tpl_id and tpl_id in tpl_id_map:
                            tpl_base_loc = tpl_id_map[tpl_id]
                            tpl_xml_obj = self.get_xml_obj(tpl_base_loc)
                            tpl_info = ContentFileParser(tpl_xml_obj, draw_params=draw_param_info)()
                            
                            # 将模板的内容合并到页面内容中
                            page_info["text_list"].extend(tpl_info.get("text_list", []))
                            page_info["img_list"].extend(tpl_info.get("img_list", []))
                            page_info["line_list"].extend(tpl_info.get("line_list", []))
                            
                            # 重新排序
                            page_info["text_list"].sort(
                                key=lambda pos_text: (float(pos_text.get("pos")[1]), float(pos_text.get("pos")[0])))
                            page_info["img_list"].sort(
                                key=lambda pos_text: (float(pos_text.get("pos")[1]), float(pos_text.get("pos")[0])))
                            page_info["line_list"].sort(
                                key=lambda pos_text: (float(pos_text.get("pos")[1]), float(pos_text.get("pos")[0])))
